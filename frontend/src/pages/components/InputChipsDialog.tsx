...
export function InputChipsAmountDialog(props: ConfirmationDialogRawProps) {
  ...
  const compuateTotalAmount = () =>
    _.entries(chipsMap).reduce((acc, item) => {
  ...
  const handleChange = (
    ...
    const newValue = parseInt((event.target as HTMLInputElement).value) || null;
    const newChipsMap = _.assign(chipsMap, { [chip]: newValue });
        ...
        {chips.map((chip) => {
          return (
            ...
            >
              ...
              <TextField
                id={`chip_${chip}`}
                label="the number of chips"
                value={_.get(chipsMap, chip) || '' }
                variant="standard"
